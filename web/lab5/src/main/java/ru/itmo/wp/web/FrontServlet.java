...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        Method method = null;
        Class<?>[] typesParameters  = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            Method[] methods = clazz.getDeclaredMethods();
            for (int i = 0; i < methods.length && method == null; i++) {
                if (methods[i].getName().equals(route.getAction())) {
                    method = methods[i];
                    typesParameters = method.getParameterTypes();
                }
            }
        }
        ...
        try {
            method.invoke(page, Arrays.stream(typesParameters).map((parameter) -> (parameter.equals(HttpServletRequest.class) ? request : view)).toArray());
        ...
        HttpSession session = request.getSession();
        String simplePageName = pageClass.getSimpleName();
        String lang;
        Template template;
        ...
        try {
            template = newTemplate(simplePageName + lang + ".ftlh");
        } catch (Exception ignored) {
            template = newTemplate(simplePageName + ".ftlh");
        }
