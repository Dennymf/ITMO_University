...
happyExpList = Happy_Data_Array.listArray (0,48) ([43008,20480,2049,44032,32768,10,21,0,32768,69,0,672,1344,2688,5376,10752,22208,0,2816,512,0,0,45056,21,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseHead","Head","Exprs","Expr","Proof","'&'","'|'","\"|-\"","\"->\"","'!'","','","'('","')'","var","%eof"]
...
happyReduction_1 ((HappyAbsSyn7  happy_var_4) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (Head (if happy_var_1 == [] then Empty else Exprs happy_var_1) happy_var_3 happy_var_4
	) `HappyStk` happyRest
